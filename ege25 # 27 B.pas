...
begin
  ...
  // кластер k3
  var (x3_min, y3_min) := (0.0, 0.0);
  min_dist := real.MaxValue;
  s := 0.0;
  for var i := 0 to k3.count - 1 do
  begin
    for var j := 0 to k3.count - 1 do
      s += dist(k3[i][0], k3[i][1], k3[j][0], k3[j][1]);
    if s < min_dist then
    begin
      min_dist := s;
      x3_min := k3[i][0];
  ...
  var px := (x1_min + x2_min + x3_min) / 3;
  var py := (y1_min + y2_min + y3_min) / 3;
